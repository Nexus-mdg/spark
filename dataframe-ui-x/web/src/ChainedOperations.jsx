      ...
      case 'merge':
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
              <label className="block">
                <span className="block text-sm text-gray-900 dark:text-gray-100">Join keys</span>
                <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                  Simple: id,name OR mappings: customer_id=cust_id,product_code=prod_code
                </div>
                <input 
                  ...
                  placeholder="id OR customer_id=cust_id"
                />
              </label>
              <label className="block">
              ...
              <button className="px-4 py-2 bg-indigo-600 text-white rounded" onClick={() => {
                ...
                const keysInput = String(state.keys || '').trim()
                if (names.length < minDataframesRequired || !keysInput) return
                
                // Parse keys - support both formats
                let keys
                if (keysInput.includes('=')) {
                  // New format with mappings: "customer_id=cust_id,product_code=prod_code"
                  keys = keysInput.split(',').map(pair => {
                    const [left, right] = pair.split('=').map(s => s.trim())
                    return { left, right }
                  }).filter(mapping => mapping.left && mapping.right)
                } else {
                  // Old format: simple column names "id,name"
                  keys = keysInput.split(',').map(s => s.trim()).filter(Boolean)
                }
