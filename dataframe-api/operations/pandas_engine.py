...
def pandas_compare(df1: pd.DataFrame, df2: pd.DataFrame, n1: str, n2: str) -> dict:
    ...
    # Schema comparison
    schema_match = (
        list(df1.columns) == list(df2.columns) and 
        list(map(str, df1.dtypes)) == list(map(str, df2.dtypes))
    )
    if not schema_match:
        return {
            'success': True, 
...
def pandas_merge(dfs: List[pd.DataFrame], names: List[str], keys: List[str], 
...
def pandas_groupby(df: pd.DataFrame, by: List[str], aggs: Optional[Dict] = None) -> pd.DataFrame:
    ...
    if isinstance(grouped.columns, pd.MultiIndex):
        grouped.columns = ['__'.join([str(x) for x in tup if str(x) != '']) 
...
def pandas_pivot(df: pd.DataFrame, mode: str, **kwargs) -> pd.DataFrame:
    ...
    if mode == 'wider':
        ...
        pivoted = pd.pivot_table(df, index=index or None, columns=names_from, 
                               values=values_from, aggfunc=aggfunc)
        
        # Flatten MultiIndex columns if any
        if isinstance(pivoted.columns, pd.MultiIndex):
            pivoted.columns = ['__'.join([str(x) for x in tup if str(x) != '']) 
...
def pandas_mutate(df: pd.DataFrame, target: str, expr: str, mode: str = 'vector', 
