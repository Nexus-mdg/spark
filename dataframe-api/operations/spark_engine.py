...
def _get_spark_session():
    ...
    builder = (
        SparkSession.builder.appName("UI-Operations")
        .master(master_url)
        ...
        if "distutils" in error_msg or "No module named" in error_msg:
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Failed to convert Spark DataFrame to pandas: {error_msg}. Fallback conversion also failed: {str(fallback_error)}. This may be due to missing system dependencies like distutils in the Spark environment."
                    )
...
def spark_groupby(df: pd.DataFrame, by: List[str], aggs: Optional[Dict] = None) -> pd.DataFrame:
    ...
    try:
        ...
        if aggs:
            from pyspark.sql.functions import avg, count, first, last, max, min, sum
                ...
                if isinstance(agg_func, list):
                    for func in agg_func:
                        if func in spark_agg_map:
                            agg_exprs.append(
                                spark_agg_map[func](col_name).alias(f"{col_name}_{func}")
                            )
                        else:
...
def spark_pivot(df: pd.DataFrame, mode: str, **kwargs) -> pd.DataFrame:
    ...
    try:
        ...
        if mode == "wider":
            ...
            from pyspark.sql.functions import avg, count, first, max, min, sum
...
def spark_datetime(df: pd.DataFrame, action: str, source: str, **kwargs) -> pd.DataFrame:
    ...
    try:
        ...
        from pyspark.sql.functions import col, date_format, dayofmonth, month, to_timestamp, year
        ...
        if action == "parse":
            ...
            if want_year_month:
                if month_style in ("short_lower", "abbr_lower", "short-lower"):
                    ...
                    sdf = sdf.withColumn(
                        cname_year_month, lower(date_format(timestamp_col, "yyyy-MMM"))
                    )
    ...
    try:
        ...
        if mode == "vector":
            ...
            try:
                ...
                spark_expr = (
                    expr.replace("col(", "").replace(")", "").replace("'", "`").replace('"', "`")
                )
