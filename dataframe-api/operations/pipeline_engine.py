...
from operations.engine_router import (
    route_merge, route_filter, route_groupby, route_select, route_rename, 
    route_pivot, route_datetime, route_mutate, route_compare, validate_engine
)
from utils.helpers import df_to_records_json_safe


def _apply_op(df_curr: pd.DataFrame | None, step: dict, preview_mode: bool = False, engine: str = 'pandas') -> tuple[pd.DataFrame, str]:
    ...
    if op == 'merge':
        ...
        # Debug logging
        print(f"[DEBUG] merge operation: how={how}, keys={keys}, left_on={left_on}, right_on={right_on}, names={names}, others={others}, engine={engine}")
        ...
        if names and df_curr is not None:
            ...
            if left_on and right_on:
                return result, f'merge current + names={names} how={how} left_on={left_on} right_on={right_on} [{engine} engine]'
            else:
                return result, f'merge current + names={names} how={how} keys={keys} [{engine} engine]'
        ...
        if names:
            ...
            for nm in names:
                try:
                    ...
                    dfs.append(df)
                    print(f"[DEBUG] loaded dataframe '{nm}': shape={df.shape}, columns={list(df.columns)}")
                except ValueError as e:
                    raise ValueError(f'merge: failed to load dataframe "{nm}": {str(e)}')
            ...
            if left_on and right_on:
                return result, f'merge names={names} how={how} left_on={left_on} right_on={right_on} [{engine} engine]'
            else:
        ...
        if left_on and right_on:
            return result, f'merge with={others} how={how} left_on={left_on} right_on={right_on} [{engine} engine]'
        else:
    ...
    if op == 'pivot':
        ...
        if mode == 'wider':
            ...
            result = route_pivot(df, mode, engine, 
                               index=index, names_from=names_from, 
                               values_from=values_from, aggfunc=aggfunc)
            
            return result, f"pivot wider names_from={names_from} values_from={values_from} [{engine} engine]"
    ...
    if op == 'compare':
        ...
        if action == 'identical':
            if result_data.get('identical'):
                ...
                return pd.DataFrame({'__note__': ['not identical']}), f'compare: identical check [{engine} engine]'
        ...
        if not result_data.get('success') or result_data.get('identical'):
            return pd.DataFrame({'__note__': ['no differences found']}), f'compare: mismatch rows [{engine} engine]'
        
        # For data mismatch, create a summary dataframe
        if result_data.get('result_type') == 'schema_mismatch':
            return pd.DataFrame({'__note__': ['schema mismatch']}), f'compare: schema mismatch [{engine} engine]'
        elif result_data.get('result_type') == 'row_count_mismatch':
            return pd.DataFrame({'__note__': ['row count mismatch']}), f'compare: row count mismatch [{engine} engine]'
            ...
            summary_df = pd.DataFrame({
                'side': ['current', other_name],
                'unique_rows': [left_unique, right_unique]
            })
    ...
    if op == 'datetime':
        ...
        if action == 'parse':
            ...
            result = route_datetime(df, action, source, engine,
                                  format=fmt, errors=errors, target=target, overwrite=overwrite)
            
            if target and target != source:
                return result, f"datetime parse {source}->{target} ({fmt or 'auto'}) [{engine} engine]"
            ...
            # Use engine router for datetime operation
            result = route_datetime(df, action, source, engine,
                                  outputs={'year': want_year, 'month': want_month, 
                                          'day': want_day, 'year_month': want_year_month},
                                  names=names, month_style=month_style, overwrite=overwrite)
    ...
    if op == 'chain_pipeline':
        ...
        print(f"[DEBUG] chain_pipeline: executing pipeline '{pipeline_name}' with input shape={df_curr.shape}, engine={engine}")
        ...
        print(f"[DEBUG] chain_pipeline: found {len(chained_steps)} steps to execute with engine={engine}")
        ...
        for i, chained_step in enumerate(chained_steps):
            print(f"[DEBUG] chain_pipeline: executing step {i+1}: {chained_step.get('op', 'unknown')} with engine={engine}")
            current, step_desc = _apply_op(current, chained_step, preview_mode, engine)  # Pass engine to chained operations
        ...
        if preview_mode:
            ...
            return df_curr, f'chain_pipeline: {pipeline_name} (preview: applied {len(chained_steps)} steps, result shape: {current.shape}) [{engine} engine]'
        else:
            ...
            return df_curr, f'chain_pipeline: {pipeline_name} (saved as {result_name}, applied {len(chained_steps)} steps) [{engine} engine]'
