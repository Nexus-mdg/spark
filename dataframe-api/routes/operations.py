...
from operations.engine_router import route_compare, route_merge, route_filter, route_groupby, route_select, route_rename, route_pivot, route_datetime, route_mutate, create_engine_response
...
def op_compare():
    ...
    try:
        ...
        return jsonify({'success': False, 'error': str(e), 'engine': engine if 'engine' in locals() else 'unknown'}), 500
...
def op_pivot():
    ...
    try:
        ...
        if mode == 'wider':
            ...
            description = f"Pivot wider from {kwargs['names_from']} values {values_from} [{engine} engine]"
...
def op_datetime():
    ...
    try:
        ...
        if action == 'parse':
            ...
            kwargs['month_style'] = (p.get('month_style') or opts.get('month_style') or 'short').lower()
