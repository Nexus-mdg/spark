            ...
            msgs.append(
                {
                    ...
                    "preview": df_to_records_json_safe(current.head(max_rows)),
                }
            )
            ...
            jsonify(
                {
                    ...
                    "error": error_msg,
                    "engine": engine,
                    "debug_info": {"error_type": type(e).__name__, "engine_used": engine},
                }
            ),
            200,
        )  # Return 200 with error details instead of 400
...
def pipeline_run():
    ...
    try:
        ...
        if current is None:
            return (
                jsonify(
                    {
                        "success": False,
                        "error": "Nothing to run: pipeline has no start and no steps",
                        "engine": engine,
                    }
                ),
                200,
        ...
        if materialize:
            ...
            meta = _save_df_to_cache(
                uniq,
                current,
                description=f"pipeline result [{engine} engine]",
                source="ops:pipeline",
            )
            meta["engine"] = engine
...
def pipelines_handler():
    ...
    try:
        ...
        obj = {
            ...
            "tags": p.get("tags") or [],
            "created_at": (
                now if not exists else json.loads(redis_client.get(key)).get("created_at", now)
            ),
            "updated_at": now,
        }
    ...
    try:
        ...
        if materialize:
            ...
            meta = _save_df_to_cache(
                uniq,
                current,
                description=f"pipeline: {name} [{engine} engine]",
                source="ops:pipeline",
            )
            meta["engine"] = engine
            created = {"name": uniq, "metadata": meta}
